...
export default function Home() {
  ...
  const n1Op = useTransform(approachScrollMobile, [0.0, 0.15], [0, 1]);
  const l1Scale = useTransform(approachScrollMobile, [0.15, 0.3], [0, 1]);
  const n2Op = useTransform(approachScrollMobile, [0.3, 0.45], [0, 1]);
  const l2Scale = useTransform(approachScrollMobile, [0.45, 0.6], [0, 1]);
  const n3Op = useTransform(approachScrollMobile, [0.6, 0.75], [0, 1]);
  const l3Scale = useTransform(approachScrollMobile, [0.75, 0.9], [0, 1]);
  const n4Op = useTransform(approachScrollMobile, [0.9, 1.0], [0, 1]);
    ...
    <main className="relative bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-50 transition-colors duration-500" suppressHydrationWarning>
      ...
      <section ref={targetRef} className="relative z-10 w-full h-[85dvh] md:h-[100dvh] flex items-center justify-center overflow-hidden">
        <div className="relative w-full h-full max-w-[1600px] max-h-[850px] md:max-h-[900px] flex items-center justify-center mx-auto">
          <motion.div style={{ y: parallaxBackdrop }} className="absolute top-[24%] md:top-[8%] lg:top-[6%] left-1/2 -translate-x-1/2 w-full text-center z-0 pointer-events-none select-none px-2">
            <motion.h1
      ...
      {/* ========================================= */}
      <section className="relative z-10 py-16 md:py-20 border-t border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/30 overflow-hidden flex flex-col transition-colors duration-500">
        <FadeIn direction="up">
        ...
        <div className="hidden sm:flex relative w-full h-[400px] overflow-hidden mask-[linear-gradient(to_right,transparent,black_5%,black_95%,transparent)] mt-8 justify-center items-center">
          <div className="relative w-[1200px] h-[400px] shrink-0 pointer-events-none">
            <CoasterItem item={toolsData[0]} index={0} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
            <CoasterItem item={toolsData[1]} index={1} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
            <CoasterItem item={toolsData[2]} index={2} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
            <CoasterItem item={toolsData[3]} index={3} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
            <CoasterItem item={toolsData[4]} index={4} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
            <CoasterItem item={toolsData[5]} index={5} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
            <CoasterItem item={toolsData[6]} index={6} totalItems={toolsData.length} coasterProgress={coasterProgress} setIsCoasterHovered={setIsCoasterHovered} />
          </div>
        </div>

        {/* --- MOBILE: MENDATAR SEPERTI AWAL --- */}
      ...
      {/* ========================================= */}
      <section ref={approachScrollRef} className="relative z-10 bg-neutral-50 dark:bg-[#0a0a0a] transition-colors duration-500 h-[250vh]">
        ...
        {/* Wrapper sticky yang diam di viewport */}
        <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
          <div className="text-center mb-2 md:mb-6 relative z-20 px-6 md:px-8 shrink-0">
          ...
          {/* TINGGI KANVAS DIPERBESAR KE 600px, POSISI Y DISESUAIKAN KE 400 & 200 AGAR TEKS TIDAK TERPOTONG */}
          <div className="hidden sm:block relative w-full h-[600px] overflow-hidden mask-[linear-gradient(to_right,transparent,black_5%,black_95%,transparent)]">
            <motion.div style={{ x: roadXTransform }} className="absolute top-0 left-0 w-[1800px] h-full flex items-center">
              <svg viewBox="0 0 1800 600" className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-sm">
                <defs>
                  {/* MASKING: Untuk memastikan animasi "charging" hanya terlihat pada garis biru yang sudah terisi */}
                  <mask id="progressMask">
                    <motion.path
                      d="M 0 400 C 150 400, 200 500, 350 500 C 500 500, 600 300, 750 300 C 900 300, 1000 500, 1150 500 C 1300 500, 1400 300, 1550 300 C 1700 300, 1750 400, 1800 400"
                      fill="none"
                      stroke="white"
                      strokeWidth="60" /* Cukup lebar untuk menutupi seluruh garis */
                      strokeLinecap="round"
                ...
                {/* 1. Base Road (Jalan Dasar abu-abu transparan) */}
                <path
                  d="M 0 400 C 150 400, 200 500, 350 500 C 500 500, 600 300, 750 300 C 900 300, 1000 500, 1150 500 C 1300 500, 1400 300, 1550 300 C 1700 300, 1750 400, 1800 400"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="16"
                  strokeLinecap="round"
                  className="text-blue-900/10 dark:text-blue-900/30"
                />

                {/* 2. Solid Blue Line (Garis Progress BIRU SOLID yang mengisi saat scroll) */}
                <motion.path
                  d="M 0 400 C 150 400, 200 500, 350 500 C 500 500, 600 300, 750 300 C 900 300, 1000 500, 1150 500 C 1300 500, 1400 300, 1550 300 C 1700 300, 1750 400, 1800 400"
                  fill="none"
                  stroke="#3b82f6" /* KONSISTEN WARNA BIRU (blue-500) */
                  strokeWidth="16"
                  strokeLinecap="round"
                  style={{ pathLength: roadScrollProgress }}
                />

                {/* 3. Animasi "Loading/Isi Daya" (Garis putih putus-putus yang bergerak MENGALIR MAJU) */}
                {/* Garis ini di-mask agar HANYA muncul di area yang sudah dilalui garis progress biru */}
                <path
                  d="M 0 400 C 150 400, 200 500, 350 500 C 500 500, 600 300, 750 300 C 900 300, 1000 500, 1150 500 C 1300 500, 1400 300, 1550 300 C 1700 300, 1750 400, 1800 400"
                  fill="none"
                  stroke="#ffffff"
                  ...
                  className="opacity-70 animate-charging"
                />
              </svg>

              {/* NODE 01 (Lembah - Y: 500) */}
              <motion.div
                className="absolute top-[500px] left-[350px] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group"
                style={{
                  opacity: useTransform(roadScrollProgress, [0.1, 0.18], [0, 1]),
                  scale: useTransform(roadScrollProgress, [0.1, 0.18], [0.8, 1]),
                }}
              >
                {/* Wajib warna border biru konsisten (border-blue-500) */}
                <div className="w-14 h-14 md:w-16 md:h-16 bg-white dark:bg-neutral-900 border-[6px] border-blue-500 rounded-full flex items-center justify-center shadow-md transition-transform duration-500 group-hover:scale-110 z-10">
                ...
                <div className="absolute top-[calc(100%+12px)] text-center w-48 md:w-56">
                  {/* Teks penanda biru konsisten */}
                  <span className="text-[10px] font-bold tracking-widest text-blue-500 uppercase">Step 01</span>
                  <h3 className="text-sm md:text-base font-bold text-neutral-900 dark:text-neutral-50 transition-colors duration-500">System Analysis</h3>
                  <p className="text-neutral-500 dark:text-neutral-400 text-[11px] md:text-xs mt-0.5 leading-relaxed transition-colors duration-500">Deconstructing user flows, database structures, and constraints.</p>
                </div>
              </motion.div>

              {/* NODE 02 (Puncak - Y: 300) - Teks berada di atas dan SANGAT AMAN DARI POTONGAN karena kanvas 600px */}
              <motion.div
                className="absolute top-[300px] left-[750px] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group"
                style={{
              ...
              >
                ...
                <div className="absolute bottom-[calc(100%+12px)] text-center w-48 md:w-56">
                  <span className="text-[10px] font-bold tracking-widest text-blue-500 uppercase">Step 02</span>
                  <h3 className="text-sm md:text-base font-bold text-neutral-900 dark:text-neutral-50 transition-colors duration-500">UX Research</h3>
                  <p className="text-neutral-500 dark:text-neutral-400 text-[11px] md:text-xs mt-0.5 leading-relaxed transition-colors duration-500">Mapping user journeys and wireframing for frictionless navigation.</p>
                </div>
              </motion.div>

              {/* NODE 03 (Lembah - Y: 500) */}
              <motion.div
                className="absolute top-[500px] left-[1150px] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group"
                style={{
              ...
              >
                ...
                <div className="absolute top-[calc(100%+12px)] text-center w-48 md:w-56">
                  <span className="text-[10px] font-bold tracking-widest text-blue-500 uppercase">Step 03</span>
                  <h3 className="text-sm md:text-base font-bold text-neutral-900 dark:text-neutral-50 transition-colors duration-500">UI Prototyping</h3>
                  <p className="text-neutral-500 dark:text-neutral-400 text-[11px] md:text-xs mt-0.5 leading-relaxed transition-colors duration-500">Translating wireframes into interactive, high-fidelity designs.</p>
                </div>
              </motion.div>

              {/* NODE 04 (Puncak - Y: 300) */}
              <motion.div
                className="absolute top-[300px] left-[1550px] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group"
                style={{
              ...
              >
                <div className="w-14 h-14 md:w-16 md:h-16 bg-white dark:bg-neutral-900 border-[6px] border-blue-500 rounded-full flex items-center justify-center shadow-md transition-transform duration-500 group-hover:scale-110 z-10">
                  <svg className="w-8 h-8 relative top-px text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                  </svg>
                </div>
                <div className="absolute bottom-[calc(100%+12px)] text-center w-48 md:w-56">
                  <span className="text-[9px] font-bold tracking-widest text-blue-500 uppercase">Step 04</span>
                  <h3 className="text-sm md:text-base font-bold text-neutral-900 dark:text-neutral-50 transition-colors duration-500">Dev Handoff</h3>
                  <p className="text-neutral-500 dark:text-neutral-400 text-[11px] md:text-xs mt-0.5 leading-relaxed transition-colors duration-500">Delivering comprehensive design systems to accelerate execution.</p>
                </div>
              </motion.div>
            </motion.div>
          </div>

          {/* --- MOBILE: ZIG-ZAG SEPERTI AWAL NAMUN WARNA BIRU KONSISTEN --- */}
          <div ref={approachRef} className="sm:hidden w-full max-w-[340px] mx-auto flex flex-col relative z-10 pt-2 pb-16">
