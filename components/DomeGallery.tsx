...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const updateAutoRotate = () => {
      if (!draggingRef.current && !inertiaRAF.current && !openingRef.current && rootRef.current?.getAttribute('data-enlarging') !== 'true') {
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotateSpeed);
        rotationRef.current = { x: 0, y: nextY };
        applyTransform(0, nextY);
      }
      rafId = requestAnimationFrame(updateAutoRotate);
    };
    ...
    <>
      ...
      >
        ...
        >
          <div className="stage">
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                ...
                >
                  <div
                    className="item__image absolute block overflow-hidden cursor-pointer bg-neutral-800 transition-transform duration-300"
                    role="button"
                    tabIndex={0}
          ...
          {/* Fade Atas */}
          <div
            className="absolute left-0 right-0 top-0 h-[150px] z-[5] pointer-events-none"
            style={{ background: `linear-gradient(to bottom, var(--overlay-blur-color, ${overlayBlurColor}), transparent)` }}
          />
          {/* Fade Bawah */}
          <div
            className="absolute left-0 right-0 bottom-0 h-[150px] z-[5] pointer-events-none"
            style={{ background: `linear-gradient(to top, var(--overlay-blur-color, ${overlayBlurColor}), transparent)` }}
          />
          {/* Fade Kiri */}
          <div
            className="absolute top-0 bottom-0 left-0 w-[100px] md:w-[200px] z-[5] pointer-events-none"
            style={{ background: `linear-gradient(to right, var(--overlay-blur-color, ${overlayBlurColor}), transparent)` }}
          />
          {/* Fade Kanan  */}
          <div
            className="absolute top-0 bottom-0 right-0 w-[100px] md:w-[200px] z-[5] pointer-events-none"
            style={{ background: `linear-gradient(to left, var(--overlay-blur-color, ${overlayBlurColor}), transparent)` }}
          />
